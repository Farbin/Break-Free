...
Term Project of Level 1 Term 2

Created By :
Afrina Tabassum and Farbin Fayza
*/
...
int textX[5]={ 100, 70, 50, 50, 100},boxDy=20, blue[6];
int maxSpeed = 18 ;
...
void obstacleMove (void);//controls the obstacles, as well as the screen, apple, brick and lifeBonus.
void bulletMove (void);//controls the bullet
...
void saveIfHighScore(void);//if the current player scores more than any of the previous high scorers, saves the new score
void pauseAll(void);//pauses all the timers
void resumeAll (void);//resumes all the timers
void startGame (void);//performs all the things to start the game
void saveGame (void);//saves the current state of the game
void loadGame (void);//loads the previously saved game
...
void takeName (void);//sets the mode for taking name input
void endForest (void);//shows the ending part
void finalScore(void);//the final score is score + (remaining lifeBonus)*1000;
int ifPrevSaved (void);//check if there exixts any previouly saved datas or not


/*
#### ABOUT THE VARIABLE "CONTROL" ####

Control is the variable to control the tasks for different pages.

Control = 0   : The main menu
Control = 1   : The game
Control = 10  : New game or saved game (First intermediate stage between control 1 and control 0)
Control = 100 : Name input  (Second intermediate stage between control 1 and control 0)
Control = 2   : Options page
Control = 3   : Instructions page
Control = 4   : High Scores
Control = 5   : Game over
Control = 6   : Credits
*/
...
void iDraw()
{
	...
	{
		iShowBMP(0,0,"All Photos/instructions1.bmp");
		if (arrowClicked==1)
			iShowBMP (0,0,"All Photos/instructions2.bmp");
	}
	...
	if (control==5) //Game Over
	{
		if (won!=1){
			...
			iText(425,300,"YOUR FINAL SCORE IS ", GLUT_BITMAP_TIMES_ROMAN_24);

			iText(725,300,count, GLUT_BITMAP_TIMES_ROMAN_24);
			if (ifHigh==1){
				iSetColor(255,255,255);
			...
			iText(400,280,"YOUR FINAL SCORE   = ", GLUT_BITMAP_TIMES_ROMAN_24);
...
void obstacleMove ()//controls the obstacles (and all the moving things)
{
	...
	for (i=0; i<screenParts ; i++)
    {
	    moveX[i]-=obsDX;
	    if (moveX[i]<=0 && i==0)
		    moveX[i]=moveX[11]+partW-maxSpeed-1;
...
void iMouse(int button, int state, int mx, int my)
{
	...
	if (control==0)//Mouse actions on the main menu page
	{
		...
		for (i=0 ; i<5 ; i++)
		{
			...
			if (mx>creditX && mx<creditX+100 && my>creditY && my<creditY+30)//selects CREDITS
			{
				creditX+=3;
				creditY+=3;
				blue[5] = 0 ;
	...
	if (control==10)//play the previously saved game ?
	{
			if (button==GLUT_LEFT_BUTTON && state==GLUT_DOWN)
			{
